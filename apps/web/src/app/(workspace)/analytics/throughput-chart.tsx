  ...
  if (data.length === 0) {
    return (
      <ChartShell title="Throughput" subtitle="Marks opened and resolved per day">
        <p className="px-4 py-10 text-center text-ui-sm text-ink-3">
  ...
  return (
    ...
    >
      <div className="px-4 pb-3 pt-1">
        ...
        <div className="mt-2 flex justify-between text-ui-xs tabular-nums text-ink-3">
  ...
  return (
    ...
    >
      <header className="flex flex-wrap items-baseline justify-between gap-3 px-3 py-2.5">
        <div>
          <h2 className="text-ui-md font-semibold text-ink">{title}</h2>
          {subtitle ? <p className="text-ui-xs text-ink-3">{subtitle}</p> : null}
        </div>
...
function LegendDot({ color, label }: { color: "mark" | "ok"; label: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-ui-xs text-ink-2">
